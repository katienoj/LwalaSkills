...
function sub_modules($parent_module, $group_id, $user_id)
{
	...
	<ul id="mainmenu" style="width:100%;">
		<?php
        //Loop throught the database to get menus that call the selected module 'my parent'
...
function CaptureTransaction($UserId, $DR, $CR, $Amount, $Currency, $DepartmentId, $Description, $TransactionType, $ChartCategory)
{
    global $conn;
    $AccountId="";
    $TimeStamp = time();
    $TransactionDate = date('Y-m-d', time());
    $ChartAccountId = AccountParentChart($AccountId);
    $TransactionType = TypeOfAccount($AccountId);
    $FinancialYear = GetFinancialPeriod();
    $FinancialMonth = GetFinancialMonth();
    $sql = mysqli_query($conn, "INSERT INTO GlobalConsolidatedTable(DepartmentId,TransactionDate,Description,Amount,Currency,UserId,FinancialYear,FinancialPeriod,SystemGeneratedStatus,TransactionType,ChartCategory,DR,CR,UserId) VALUES('$DepartmentId','$TransactionDate','$Description','$Amount','$Currency','$UserId','$FinancialYear','$FinancialMonth','1','$TransactionType','$ChartCategory','$DR','$CR','$UserId')") or die(mysqli_error($conn));
    if ($sql == 1) {
        $InitiatiorTransaction = mysqli_insert_id($conn);
        $AffectedAccounts = explode(':', GetLinkedToAccounts($AccountId));
        if ($DR == '') {
            $Credit = $CR;
            $Debit = '';
        } elseif ($CR == '') {
        ...
        foreach ($AffectedAccounts as $AffectedAccount) {
            if ($AffectedAccount != '') {
                $sql = mysqli_query($conn, "INSERT INTO GlobalConsolidatedTable(DepartmentId,TransactionDate,Description,Amount,Currency,UserId,FinancialYear,FinancialPeriod,SystemGeneratedStatus,TransactionType,ChartCategory,DR,CR,UserId,ConnectedDoubleEntryAccount) VALUES('$DepartmentId','$TransactionDate','$Description','$Amount','$Currency','$UserId','$FinancialYear','$FinancialMonth','1','$TransactionType','$ChartCategory','$Debit','$Credit','$UserId','$AffectedAccount')") or die(mysqli_error($conn));
            }
        }
    }
}
...
function InsertIntoChargeSheet($ChargeId, $DepartmentId, $Particulars, $EpisodeNo, $Cost, $qty, $Amount, $PatientId, $Date, $UserId, $Currency, $Service, $BillType)
{
    global $conn;
    $sql = mysqli_query($conn, "INSERT INTO PatientChargeSheet(BillingStatus,DepartmentId,ParticularsId,EpisodeNo,Cost,Quantity,Amount,PatientId,Date,UserId,Currency,Type,BillType) VALUES(0,'$DepartmentId','$Particulars','$EpisodeNo','$Cost','$qty','$Amount','$PatientId','$Date','$UserId','$Currency','$Service', '$BillType')") or die(mysqli_error($conn));
    if ($sql == 1) {
...
function GetActualChartAmount($FinalChartIds, $ChartCurrency, $ChartFinancialYear, $ChartFinancialPeriod)
{
    ...
    foreach ($ChartIds as $ChartId) {
        if ($ChartId != '') {
            ...
            while ($recs = mysqli_fetch_array($sqlAmt)) {
                $Amount = $recs['Amount'];
...
function FetchChartAccounts($Id, $ChartCurrency, $ChartFinancialYear, $ChartFinancialPeriod)
{
    ...
    while ($recs = mysqli_fetch_array($sql)) {
		...
		<li><a href="#" onmousedown="ShowAccounts(<?php echo $AccountId; ?>,1)" ondblclick="LoadChartSubMenu('<?php echo $AccountId; ?>',event)" style="text-decoration:none;color:#990000;"><?php echo $AccountName; ?></a>
			<div id="ChartAmtTotal<?php echo $Id; ?>" style="margin-top:0px; margin-bottom:0px; background-color:#E2FFE9; border:solid 1px #0000FF; float:right; width:100px; left:10px; position:relative;"><?php echo  CurrencyCode($ChartCurrency) . " " . FetchTotalChartAmt($AccountId, $ChartCurrency, $ChartFinancialYear, $ChartFinancialPeriod); ?></div>
			<ul><?php echo  FetchChartAccounts($AccountId, $ChartCurrency, $ChartFinancialYear, $ChartFinancialPeriod); ?></ul>
		</li>
	<?php
    }
}
function FetchChartAccountsCheckBox($Id, $ChartCurrency, $ChartFinancialYear, $ChartFinancialPeriod)
{
    global $conn;
    $sql = mysqli_query($conn, "SELECT * FROM  ChartOfAccounts WHERE ParentChartAccount='$Id'") or die(mysqli_error($conn));
    $count = 0;
    while ($recs = mysqli_fetch_array($sql)) {
		...
		<li style="background-color:<?php echo $bg; ?>"><a href="#" onmousedown="ShowAccounts(<?php echo $AccountId; ?>,1)" ondblclick="LoadChartSubMenu('<?php echo $AccountId; ?>',event)" style="text-decoration:none;color:#990000;"><?php echo $AccountName; ?></a>
			<div id="ChartAmtTotal<?php echo $Id; ?>" style="margin-top:-20px; margin-bottom:-20px; float:right; width:100px;width:130px;"><input type="checkbox" name="ChartCheck" id="ChartCheck" value="<?php echo $AccountId; ?>" onclick="FetchSelectedChartBoxes()" /></div>
			<ul><?php echo  FetchChartAccountsCheckBox($AccountId, $ChartCurrency, $ChartFinancialYear, $ChartFinancialPeriod); ?></ul>
		</li>
	<?php
        $count++;
    }
}
function FetchChartAccountsDummy($Id, $ChartCurrency, $ChartFinancialYear, $ChartFinancialPeriod)
{
    global $conn;
    $sql = mysqli_query($conn, "SELECT * FROM  ChartOfAccountsDummy WHERE ParentChartAccount='$Id'") or die(mysqli_error($conn));
    $count = 0;
    while ($recs = mysqli_fetch_array($sql)) {
        $AccountId = $recs['Id'];
        $AccountName = $recs['AccountName'];
		...
		<li style="background-color:<?php echo $bg; ?>"><a href="#" onmousedown="ShowAccounts(<?php echo $AccountId; ?>,1)" ondblclick="LoadChartSubMenu('<?php echo $AccountId; ?>',event)" style="text-decoration:none;color:#990000;"><?php echo $AccountName; ?></a>
			<div id="ChartAmtTotal<?php echo $Id; ?>" style="margin-top:-20px; margin-bottom:-20px; float:right; width:100px;width:130px;"><input type="checkbox" name="ChartCheck" id="ChartCheck" value="<?php echo $AccountId; ?>" onclick="FetchSelectedChartBoxes()" /></div>
			<ul><?php echo  FetchChartAccountsDummy($AccountId, $ChartCurrency, $ChartFinancialYear, $ChartFinancialPeriod); ?></ul>
		</li>
	<?php
        $count++;
    }
...
function FetchChartSubAccountsCheckBox($AccountTypeId, $AccountId)
{
    global $conn;
    $sql = mysqli_query($conn, "SELECT * FROM  FinancialSubAccounts WHERE ParentAccount='$AccountId'") or die(mysqli_error($conn));
    while ($recs = mysqli_fetch_array($sql)) {
...
function ChartAccountName($AccountId)
{
    global $conn;
    $sql = mysqli_query($conn, "SELECT * FROM ChartOfAccounts WHERE Id='$AccountId'") or die(mysqli_error($conn));
    $result = mysqli_fetch_array($sql);
    $AccountName = $result['AccountName'];
    return $AccountName;
}
function AccountParentChart($Account)
{
    global $conn;
    $sql = mysqli_query($conn, "SELECT ChartAccountId FROM FinancialAccounts WHERE Id='$Account'") or die(mysqli_error($conn));
    $result = mysqli_fetch_assoc($sql);
    $ChartAccountId = $result['ChartAccountId'];
    return $ChartAccountId;
}
function TypeOfAccount($Account)
{
    global $conn;
    $sql = mysqli_query($conn, "SELECT TypeOfAccount FROM FinancialAccounts WHERE Id='$Account'") or die(mysqli_error($conn));
    $result = mysqli_fetch_assoc($sql);
    $TypeOfAccount = $result['TypeOfAccount'];
    return $TypeOfAccount;
}
function GetAccountCategoryName($AccountCategoryId)
{
    global $conn;
    $ExecSqlDStatement = mysqli_query($conn, "Select CategoryName From AccountsCategories Where Id = '$AccountCategoryId'") or die(mysqli_error($conn));
    $GetAccountDetails = mysqli_fetch_assoc($ExecSqlDStatement);
    return htmlentities($GetAccountDetails['CategoryName']);
}
function GetTransactionTypeName($TransactionTypeId)
{
    global $conn;
    $ExecSqlDStatement = mysqli_query($conn, "Select TypeName From AccountsTransactionTypes Where Id = '$TransactionTypeId'") or die(mysqli_error($conn));
    $GetAccountDetails = mysqli_fetch_assoc($ExecSqlDStatement);
    return htmlentities($GetAccountDetails['TypeName']);
}
function GetPostableCharts($ModuleId)
{
    global $conn;
    $sqlGetConnectedCats = mysqli_query($conn, "SELECT * FROM ModuleAccountsCategories WHERE ModuleId='$ModuleId'") or die(mysqli_error($conn));
    while ($recs = mysqli_fetch_array($sqlGetConnectedCats)) {
        $CategoryId = $recs['CategoryId'];
        $sqlLinkedCategories = mysqli_query($conn, "SELECT * FROM ChartOfAccounts WHERE AccountCategory='$CategoryId'") or die(mysqli_error($conn));
        while ($recsCat = mysqli_fetch_array($sqlLinkedCategories)) {
            $ChartAccount = $recsCat['Id'];
            $AccountName = $recsCat['AccountName'];
            $sqlFinAc = mysqli_query($conn, "SELECT * FROM FinancialAccounts WHERE AccountName='$AccountName' AND ChartAccountId='$ChartAccount'") or die(mysqli_error($conn));
            if (mysqli_num_rows($sqlFinAc) == 0) {
                $PrevPostable = 0;
            } else {
                $PrevPostable = 1;
            }
            if ($PrevPostable == 1) {
                while ($recsAccounts = mysqli_fetch_array($sqlFinAc)) {
                    $AccountId = $recsAccounts['Id'];
                    $FinAcName = $recsAccounts['AccountName'];
                    echo "<option value='$AccountId'>$FinAcName</option>";
                }
            }
        }
    }
