     ...
     <!-- START SIDE BAR -->
      ...
      <aside class="main-sidebar sidebar-dark-primary elevation-4">
        ...
        <a href="../admindashboard/index3.html" class="brand-link">
          ...
          <span class="brand-text font-weight-light">CHW Requisition</span>
        </a>
        <!-- Sidebar -->
        <div class="sidebar">
...
<footer valign="bottom">
   ...
   <?php
            $sql=mysqli_query($conn, "SELECT EmployeeTable.EmployeeName, COUNT(PatientId) as patientInvestigations FROM PatientInvestigationRequest
             ...
             WHERE YEAR(PatientInvestigationRequest.Date) = '$this_year'
             AND MONTH( PatientInvestigationRequest.Date ) = '$this_month'
            ...
            while ($rec=mysqli_fetch_array($sql)) {
                ...
                echo "['".$rec['EmployeeName']."', ".$rec['patientInvestigations']."],";
    ...
    ?>
		  ...
		  var options = {
			title: 'NUMBER OF LABORATORY REQUESTS SENT IN <?php echo "MONTH: ".$this_month." YEAR: ".$this_year ?> (Nurses)'
		  };
   ...
   $today =  date('Y-m-d');
            $sql=mysqli_query($conn, "SELECT EmployeeTable.EmployeeName, COUNT(PatientId) as patientPrescription FROM PatientPrescription
             ...
             JOIN UsersTable ON EmployeeTable.Id  = UsersTable.EmployeeId
             WHERE YEAR(PatientPrescription.Date) = '$this_year'
             AND MONTH( PatientPrescription.Date ) = '$this_month'
             AND UsersTable.GroupId = 3
            ...
            while ($rec=mysqli_fetch_array($sql)) {
                ...
                echo "['".$rec['EmployeeName']."', ".$rec['patientPrescription']."],";
    ...
    ?>
		  ...
		  var options2 = {
			title: 'NUMBER OF PRESCRIPTIONS DONE IN <?php echo "MONTH: ".$this_month." YEAR: ".$this_year ?> (Nurses)',
			pieHole: 0.4,
		...
		function drawChart3() {
            ...
            $sql=mysqli_query($conn, "SELECT EmployeeTable.EmployeeName, COUNT(PatientId) as patientDiagnosis FROM icd_10
             ...
             JOIN UsersTable ON EmployeeTable.Id  = UsersTable.EmployeeId
             WHERE YEAR(icd_10.Date) = '$this_year'
             AND MONTH( icd_10.Date ) = '$this_month'
             AND UsersTable.GroupId = 3
             GROUP BY EmployeeName ORDER BY patientDiagnosis DESC")or die(mysqli_error($conn));
            while ($rec=mysqli_fetch_array($sql)) {
                $EmployeeName=$rec['EmployeeName'];
                $patientDiagnosis=$rec['patientDiagnosis'];
                echo "['".$rec['EmployeeName']."', ".$rec['patientDiagnosis']."],";
            }
    ?>
			]);
	
			var options3 = {
			  title: 'NUMBER OF FINAL DIAGNOSIS CAPTURED IN THE <?php echo "MONTH: ".$this_month." YEAR: ".$this_year ?>(Nurses)',
			  pieHole: 0.0,
		...
		function drawChart4() {
            ...
            $sql=mysqli_query($conn, "SELECT EmployeeTable.EmployeeName, COUNT(PatientId) as patientAdmission FROM PatientAdmission
             ...
             JOIN UsersTable ON EmployeeTable.Id  = UsersTable.EmployeeId
             WHERE YEAR(PatientAdmission.Date) = '$this_year'
             AND MONTH( PatientAdmission.Date ) = '$this_month'
             AND UsersTable.GroupId = 3
             GROUP BY EmployeeName ORDER BY patientAdmission DESC")or die(mysqli_error($conn));
            while ($rec=mysqli_fetch_array($sql)) {
                $EmployeeName=$rec['EmployeeName'];
                $patientAdmission=$rec['patientAdmission'];
                echo "['".$rec['EmployeeName']."', ".$rec['patientAdmission']."],";
    ...
    ?>
			...
			var options4 = {
			  title: 'NUMBER OF PATIENT ADMISSIONS REQUESTED IN THE <?php echo "MONTH: ".$this_month." YEAR: ".$this_year ?>(Nurses)',
			  pieHole: 0.4,
  
			};
	
			var chart4 = new google.visualization.ColumnChart(document.getElementById('donutchart4'));
    ...
    google.charts.load('current', {'packages':['corechart']});
        ...
        <?php
            $sql=mysqli_query($conn, "SELECT EmployeeTable.EmployeeName, COUNT(PatientId) as patientComplaints
             ...
             JOIN UsersTable ON EmployeeTable.Id  = UsersTable.EmployeeId WHERE YEAR(PatientComplains.Date) = '$this_year'
             AND MONTH( PatientComplains.Date ) = '$this_month' AND UsersTable.GroupId = 3
             GROUP BY EmployeeName ORDER BY patientComplaints DESC")or die(mysqli_error($conn));
            while ($rec=mysqli_fetch_array($sql)) {
                $EmployeeName=$rec['EmployeeName'];
                $patientComplaints=$rec['patientComplaints'];
                echo "['".$rec['EmployeeName']."', ".$rec['patientComplaints']."],";
    ...
    ?>
			...
			var options5 = {
			  title: 'NUMBER OF PATIENT COMPLAINTS CAPTURED IN THE <?php echo "MONTH: ".$this_month." YEAR: ".$this_year ?>(Nurses)',
			  pieHole: 0,
    ...
    google.charts.load('current', {'packages':['corechart']});
        ...
        <?php
            $sql=mysqli_query($conn, "SELECT EmployeeTable.EmployeeName, COUNT(PatientId) as patientProcedures FROM PatientProcedureRequest
             ...
             JOIN UsersTable ON EmployeeTable.Id  = UsersTable.EmployeeId
             WHERE YEAR(PatientProcedureRequest.Date) = '$this_year'
             AND MONTH( PatientProcedureRequest.Date ) = '$this_month'
             AND UsersTable.GroupId = 3
             GROUP BY EmployeeName ORDER BY patientProcedures DESC")or die(mysqli_error($conn));
            while ($rec=mysqli_fetch_array($sql)) {
                $EmployeeName=$rec['EmployeeName'];
                $patientProcedures=$rec['patientProcedures'];
                echo "['".$rec['EmployeeName']."', ".$rec['patientProcedures']."],";
    ...
    ?>
			...
			var options6 = {
			  title: 'NUMBER OF PATIENT PROCEDURES REQUESTED IN THE <?php echo "MONTH: ".$this_month." YEAR: ".$this_year ?> (Nurses)',
			  pieHole: 0,
  
			};
	
			var chart6 = new google.visualization.ColumnChart(document.getElementById('donutchart6'));
