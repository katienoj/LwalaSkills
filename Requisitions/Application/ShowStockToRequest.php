  ...
  <tr>
    <td colspan="3" style="align-content: center;">
      <select style="align-content: center;" class="form-control" name="DeptName" id="DeptName" >
        <option style="align-content: center;" class="form-control" value="">Please Select Approver 
</option>
        <?php
        $sql = mysqli_query($conn, "SELECT * FROM EmployeeTable WHERE status=1 ORDER BY EmployeeName ASC") or die(mysqli_error($conn));
        while ($recs = mysqli_fetch_array($sql)) {
          $Id = $recs['Id'];
          $EmployeeName = $recs['EmployeeName'];
        ?>
          <option style="align-content: center;" class="form-control" value="<?php echo $Id; ?>"><?php echo $EmployeeName; ?></option>
        <?php
        }
        ?>
      </select>
