...
<table width="100%" border="0" bgcolor="#E4E4E4" class="formborder">
  <tr>
    <td colspan="2">
      <table width="100%" border="0" class="bordercolor2" style="background-image:url(../Main/Layout/images/form_top.jpg); background-repeat:repeat-x;">
        <tr>
          <td width="94%" class="formtop" onMouseDown="javascript:getReadyToMove('popup_div_1', event);" onMouseUp="javascript:dropLoadedObject(event)" onClick="javascript:dropLoadedObject(event);">Add suppliers to the <?php echo CatName($CatId); ?> category </td>
  ...
  ?>
    <tr>
      <td>
        <table width="100%" border="0">
          <thead>
            <tr>
              <td ><input type="checkbox" name="SupplierCheck" id="SupplierCheck" value="SupplierCheck" onClick="SelectSuppliers()"></td>
              <td >Supplier</td>
              <td >Phy Address</td>
              <td >Post Address</td>
              <td >Telephone</td>
    ...
    <tr>
      <td align="center"><input class="btn btn-info" type="button" name="inputButton" id='inputButton' value="Add suppliers" onclick="AddSuppliersToCategory()" />
      </td>
    </tr>
  <?php
  } else {
    echo "No suppliers To service this category";
  }
  ?>
</table>