...
<table width="100%" border="0">
	<?php
$sql=mysqli_query($conn, "SELECT * FROM InternalStockRequests WHERE HODApproved='1' AND Processed='1'  ORDER BY DateOfRequest DESC") or die(mysqli_error($conn));
if (mysqli_num_rows($sql)==0) {
	...
	<thead>
		<tr>
			...
			<td width="11%" class="heading">Processed </td>
		</tr>
	</thead>
    ...
    while ($recs = mysqli_fetch_array($sql)) {
        ...
        if ($count % 2 == 0) {
			...
			<tr bgColor='<?php echo $bg; ?>' onMouseOver="this.bgColor='#FFFFFF'" onMouseOut="this.bgColor='<?php echo $bg; ?>'">
				...
				<td >By <?php echo ResolveEmployeeName($ProcessedBy) . "<br> on " . dteconvert($DateOfProcessing); ?></td>
