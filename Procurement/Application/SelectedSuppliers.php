...
<td><table width="100%" border="0" class="bordercolor2" style="background-image:url(../Main/Layout/images/form_top.jpg); background-repeat:repeat-x;">
      <tr >
        <td width="94%" class="formtop"   onMouseDown="javascript:getReadyToMove('popup_div', event);" onMouseUp="javascript:dropLoadedObject(event)" onClick="javascript:dropLoadedObject(event);">Requested Suppliers </td>
...
<?php
    $sql=mysqli_query($conn, "SELECT * FROM QuotationRequests WHERE Id='$QuoteRequestId'") or die(mysqli_error($conn));
...
$result=mysqli_fetch_array($sql);
        $Suppliers=explode(':', $result['Suppliers']);
        foreach ($Suppliers as $supplier) {
            if ($supplier!='') {
                $SupplierSQL="SELECT Id, SupplierNames, PhyAddress, PostAddress, Email, Phone, WebSite, Town, Country, 
   CreditTerms, CreditLimitAmount, SupplierLogo, DateAdded, del 
   FROM SuppliersTable WHERE Id='$supplier' GROUP BY Id";
                //echo $SupplierSQL;
                $sqlSupplier=mysqli_query($conn, $SupplierSQL) or die(mysqli_error($conn));
                $count=0;
                $recs=mysqli_fetch_array($sqlSupplier);
                $Id=$recs['Id'];
