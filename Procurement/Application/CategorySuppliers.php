  ...
  <tr>
    <td colspan="2">
      <table width="100%" border="0" class="bordercolor2" style="background-image:url(../Main/Layout/images/form_top.jpg); background-repeat:repeat-x;">
        <tr>
          <td width="94%" class="formtop" onMouseDown="javascript:getReadyToMove('popup_div', event);" onMouseUp="javascript:dropLoadedObject(event)" onClick="javascript:dropLoadedObject(event);">Suppliers who supply items in the <?php echo StockCategoryName($CatId); ?> Category</td>
    ...
    <td>
      <table width="100%" border="0">
        ...
        <tbody style="width:100%;max-height:300px;overflow-x:hidden;overflow-y:auto;">
          ...
          $theSuppliers = '';
          $shown = 0;
          $SupplierSQL = "SELECT SupplierId FROM SupplierCategoryLink WHERE CatId='$CatId'";
          $sqlLink = mysqli_query($conn, $SupplierSQL) or die(mysqli_error($conn));
          ...
          foreach ($suppliers as $supplier) {
              if ($supplier != '') {
                  $SupplierSQL = "
                  SELECT  Id, SupplierNames, PhyAddress, PostAddress, Email, Phone, WebSite, Town, Country, CreditTerms, 
                  CreditLimitAmount, SupplierLogo, DateAdded, del 
                  FROM SuppliersTable WHERE Id='$supplier' GROUP BY Id";
                  //echo $SupplierSQL;
