...
<table border="0" width="100%">
  ...
  <?php
  $sqlTempPRQ = mysqli_query($conn, "SELECT * FROM ProcurementTable WHERE ApprovalStatus is Null OR ApprovalStatus=' ' order by Id DESC") or die(mysqli_error($conn));
  if (mysqli_num_rows($sqlTempPRQ) == 0) {
  ?>
    <tr>
      <td  align="center">Sorry. Lwala did not find any unapproved procurement request </td>
    </tr>
  <?php
  } else {
  ?>
