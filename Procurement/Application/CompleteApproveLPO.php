...
if(1>0)
{
	...
	if(GetUserDepartment($UserId)==10 or stristr(DepartmentName(GetUserDepartment($UserId)),"procurement") or stristr(DepartmentName(GetUserDepartment($UserId)),"Procurement") )
	{
	  ...
	  {
	       $sqlCheck=mysqli_query($conn, "SELECT LPOApproval FROM StockLPO WHERE Id='$LPOId'") or die(mysqli_error($conn));
		   ...
		   {
	      $sql=mysqli_query($conn, "UPDATE StockLPO SET LPOApproval='1',LPOApprover='$UserId',LPODateOfApproval='$Now' WHERE Id='$LPOId'") or die(mysqli_error($conn));
		  }
	  }
	  else
	  {
